/* METADATA
{
  ...
  "description": "指尖语气 Fingertips：查看输入节奏状态、隐私配置并清理本地记录。运行时会临时读取输入事件，但不会保存、上传或注入输入原文。",
  "tools": [
    {
      "name": "status",
      "description": "查看当前运行状态、实际配置门槛与本地输入节奏记录统计。不返回输入原文；运行时临时读取的输入事件不会被保存或上传。",
      "parameters": [{"name":"chat_id","description":"可选，目标对话 ID。","type":"string","required":false}]
    },
    {
      "name": "clear",
      "description": "清理指定对话或全部 Fingertips 本地记录。不读取或返回已保存的输入原文；插件不会保存输入原文。",
      "parameters": [{"name":"chat_id","description":"可选，目标对话 ID；留空清理全部记录。","type":"string","required":false}]
    },
    {
      "name": "capabilities",
      "description": "查看输入节奏工作能力、当前配置与隐私保护范围。不会修改设置；运行时会临时读取输入事件，但不会保存、上传或注入输入原文。",
      "parameters": []
    }
  ]
}
...
function summary(s) { return (s.enabled ? "Fingertips 注入已启用" : "Fingertips 注入已禁用") + "；" + (s.scope === "global" ? "作用于所有对话" : "仅作用于绑定对话") + "；" + (s.injectRhythm ? "Fingertips生成判定" + s.minNoteSec + "秒，输入停顿判定" + s.pauseGapSec + "秒" : "输入节奏已禁用") + "；超过" + s.orphanMinutes + "分钟没有继续输入或发送时，记为一段搁置的欲言又止。"; }
function instruction() { return "请优先使用 summary 向用户用中文概括结论，不要原样展示 JSON、内部状态文件路径或完整 ID 列表。"; }
var PRIVACY = "运行时会临时读取输入事件，用于判断输入状态和附加节奏摘要；不会保存、上传或注入输入原文，只记录输入时间、停顿和持续时长等节奏数据。";
